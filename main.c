 ...
 */
 unsigned char mmap_set=0;

 //function to parse the mount options

 void parse_options(void *data)
 {
 	char *opt=(char *)data;
 	char *token=opt;
 	mmap_set=0;
 ...
 static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
 {
	...
	/* allocate superblock private data */
 	...
 	((struct wrapfs_sb_info *)(sb)->s_fs_info)->mmap_set=mmap_set;
	#ifdef WRAPFS_CRYPTO

 	memset(((struct wrapfs_sb_info *)(sb)->s_fs_info)->key,'0',keylength+1);
 	((struct wrapfs_sb_info *)(sb)->s_fs_info)->key[keylength]='\0';
	#endif
