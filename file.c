...
#include <linux/string.h>
 	...
 	size_t count, loff_t *ppos)
 {
 	...
 	if(1==WRAPFS_SB(file->f_dentry->d_sb)->mmap_set)
 	{
		#ifdef WRAPFS_CRYPTO
 		if(0==memcmp(&(WRAPFS_SB(file->f_dentry->d_sb)->key),&zerokey,keylength)) {
 			return -ENOKEY;
 		}
		#endif
 	...
 	unsigned long arg)
 {
 	...
 	memset(zerokey,'0',sizeof(zerokey));
 	zerokey[keylength]='\0';
 	key=kmalloc(keylength+1,GFP_KERNEL);
 	if(!key)
 	{
 		err=-ENOMEM;
 		goto out;
 	...
 	if(copy_from_user(key,(char *)arg,keylength))
 	{
 		kfree(key);
 		goto out;
 	}
 	...
 	if(0==memcmp(zerokey,key,strlen(key)))
 		memcpy(WRAPFS_SB(file->f_dentry->d_sb)->key,zerokey,sizeof(zerokey)-1);
 	else
...
if (willwrite && !lower_file->f_mapping->a_ops->writepage) {
	 ...
	 */
	 ...
	 if(1==WRAPFS_SB(file->f_dentry->d_inode->i_sb)->mmap_set)
	file->f_mapping->a_ops = &wrapfs_mmap_aops; /* set our aops */
	 	else
